<template>
    <v-dialog v-model="dialog" max-width="500px">
        <v-card>
            <v-card-title>
                <span>
                    <v-icon class="mr-2">how_to_reg</v-icon>Novo E-mail
                </span>
                <v-spacer></v-spacer>
            ...
            <v-card-text>
                <v-text-field v-model="passwordChosen" label="Senha" type="password"></v-text-field>
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        loadDomains() {
            this.$axios.$get('/virtual-domains').then(resp => {
    ...
    computed: {
        selectedDomainName() {
            ...
            return (
                '@' +
                this.domains[
                    this.domains.findIndex(d => d.id === this.selectedDomain)
                ].name
            )
        }
    },
